...
class ApiEndpointsJobTest < ActiveJob::TestCase
  test "builds endpoint request, duration p95, and server error rollups" do
    ...
    create_api_metric(name: Metric::API_REQUEST_ERROR_COUNT, value: 1, occurred_at: window_start + 27.minutes, action: "show", status: 500)

    assert_difference "Rollup.count", 5 do
    ...
    assert_in_delta 1.0, rollup_value("observability.api.endpoint.requests", "hour", window_start, controller: "Api::V1::UsersController", action: "show"), 0.001
  ...
  test "skips rollups when endpoint dimensions are blank" do
    window_start = Time.zone.parse("2026-06-18 10:00:00 UTC")
    window_end = window_start + 1.hour
